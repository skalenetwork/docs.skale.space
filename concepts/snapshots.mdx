...
Assumptions:
1. First block on SKALE chain occurred at <Latex>TfirstBlock</Latex>
2. node does snapshot every <Latex>TsnapshotInterval</Latex>
...
1. `k+1`-th snapshot will be done once another block’s B timestamp crosses boundary <Latex>[TfirstBlockTsnapshotInterval]+1\*TsnapshotInterval</Latex>
