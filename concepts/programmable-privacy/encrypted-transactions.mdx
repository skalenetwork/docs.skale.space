...
description: "Hide transaction payloads during mempool and consensus with threshold encryption"
---
...
Encrypted Transactions hide the `to` address and `data` fields of transactions until after they execute. Once decrypted and executed, transaction details are visible onchain like any other transaction.
...
MEV bots need visible transaction data to extract value. By encrypting `to` and `data` during mempool and consensus, encrypted transactions remove the information they rely on:
...
- [API Reference](/developers/programmable-privacy/api-and-faqs) — TypeScript SDK and JSON-RPC methods
