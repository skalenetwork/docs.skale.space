...
class AIAgent {
  ...
  constructor(privateKey: string, facilitatorURL: string) {
    ...
    // Create payment tool for the agent
    const paymentTool = {
      name: 'make_payment',
...
1. **x402 Client** - Handles payment creation and settlement
2. **AI Model** - Processes requests and makes decisions
3. **HTTP Client** - Interacts with paywalled APIs
4. **Payment Tool** - Integrates payment capability into agent workflows
