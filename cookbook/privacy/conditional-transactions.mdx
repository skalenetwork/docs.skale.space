...
description: "Complete step-by-step guide to implementing a private Rock-Paper-Scissors game using BITE Conditional Transactions"
---

## Rock-Paper-Scissors with Conditional Transactions

This comprehensive tutorial walks you through building a **Rock-Paper-Scissors game** that uses BITE's Conditional Transaction (CTX) feature to keep player moves encrypted until both players have committed. The game automatically decrypts moves onchain and determines the winner without any manual reveal phase.
...
|--------|-------------|------------|
| **1. Create** | Player 1 creates a game with encrypted move and wager | Move encrypted via BITE SDK, stored onchain |
...
This tutorial requires access to a CTX-enabled SKALE chain. The demo is configured for **BITE V2 Sandbox 2**. Check [BITE Protocol Phases](/concepts/bite-protocol/phases) for current availability.
</Note>
