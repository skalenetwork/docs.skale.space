...
1. Encode the function call via [multisigwallet-cli](/skale-chain/run-a-skale-chain/using-multisig-wallet-cli)
...
2. Execute via SAFE by following the steps [here](/skale-chain/run-a-skale-chain/using-safe#submit-transaction-to-safe)
...
```shell title="MultisigWallet CLI"
npx msig encodeData [schain-name] MessageProxyForSchain registerExtraContract destination-skale-chain-name local-contract-address
```
...
![Click on Transaction Builder](/run-a-skale-chain/safe-transaction-builder.png)
...
4. Select `registerExtraContract` from the dropdown list in *Contract Method Selector*, set the *schainName (string)* field (e.g elated-tan-skat), and set the *extraContract (address)* field which is the smart contract on Ethereum
   ...
   ![Input transaction details](/skale-bridge/custom-contract-ethereum-registration.png)
