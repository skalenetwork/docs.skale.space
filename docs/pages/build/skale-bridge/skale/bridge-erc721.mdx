...
::::note
If the SKALE Chains you are mapping the ERC-721 token from/to are not connected, please [connect the SKALE Chains](/build/skale-bridge/skale/connect-schains).
...
```solidity title="InterchainERC721.sol"
// [!include ~/snippets/skale-bridge/InterchainERC721.sol
```
...
```solidity title="YourInterchainNFT.sol"
// [!include ~/snippets/skale-bridge/InterchainNFT.sol
```
...
npx msig encodeData [DST_SCHAIN_NAME] TokenManagerERC721 addERC721TokenByOwner [ORIGIN_SCHAIN_NAME] [0x_ORIGIN_TOKEN] [0x_DST_TOKEN
```
...
The following does not require you to setup your own token. This works with **ANY** ERC-721 token that is mapped from a SKALE Chain to any other SKALE Chain as long as the actual ERC-721 token on each side does not have additional restrictions around who can transfer.
...
```js title="bridge.js"
// [!include ~/snippets/skale-bridge/bridge-erc721.js
```