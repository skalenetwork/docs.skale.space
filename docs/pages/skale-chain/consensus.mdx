...
- create `block proposal` for each `block number` and broadcast it to peers, collecting $\frac{2}{3} N$ `data availability signatures` and creating `DA proofs`. (_block proposal phase_)
...
- sign the statement on which proposal won (`block signature share`) and broadcast it to other nodes. Wait until receipt of $\frac{2}{3}$ of `block signature shares` and merge them 
...
Once the `sending node` collects `DA signatures` from `$\frac{2}{3}$` of nodes, it will merge the signatures into a `DA proof`. The `DA proof` proves that the proposal has been widely distributed over the network.
...
Second, since `DA proof` creation requires a $\frac{2}{3}$ signature of nodes, the proposal is _guaranteed to be unique_. 
...
Once a node receives `DA proofs` from $\frac{2}{3}$ of nodes, the node will start the block consensus phase.
...
After `block consensus` decides on the winning block, each node will sign the statement specifying the winning proposal (`block signature share`) and broadcast it to other nodes. The node will then wait until receipt of $\frac{2}{3}$ of `block signature shares` and merge the shares into `block signature`.
...
Note that $\frac{2}{3}$ of the nodes are guaranteed to have a copy of the proposal after _DA proof phase_
...
$$3t + 1 \leq N$$
or
$t \leq \lfloor\frac{N-1}{3}\rfloor$
...
$h = N - t = N - \lfloor\frac{N-1}{3}\rfloor = \lfloor\frac{2n-1}{3}\rfloor$
