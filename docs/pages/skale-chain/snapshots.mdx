...
2. node does snapshot every $T_{snapshotInterval}$
...
1. `k+1`-th snapshot will be done once another block's B timestamp crosses boundary $\left[\frac{T_{firstBlock}}{T_{snapshotInterval}}\right] + 1 * T_{snapshotInterval}$
