...
generate their own polynomial $p_i(x)$, and let the group's
...
polynomial, each player i sends $p_i(j)$ to player j for all j.
In this way, player j can recover their share $p_1(j) + p_2(j) + \dots + p_n(j) = p(j)$.
...
$f_i(x) = a_0 + a_1x + \dots + a_tx^t$. Then,
let $g_i(x)$ be $a_0 + a_1x g + \dots + a_tx^t g$. Due to ECDLP, f_i(x) is unrecoverable from g_i(x). However, given g_i(x) and f_i(j) from player i,
you can verify that player i gave you a valid output by checking that $g_i(j) = f_i(j) g$. Further, note that the sum of all $g_{x}(x)$ results in a
...
Thus, instead of needing to trust that $p_i(j)$ is a correct output,
you first require that all players broadcast their encrypted polynomials, or public polynomials. Then, you can verify $p_i(j)$ against
$g_{x}(x)$. Finally, if this verification fails, you issue a complaint, for which you then
