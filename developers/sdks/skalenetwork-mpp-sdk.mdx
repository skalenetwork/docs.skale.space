...
Currency identifiers in this page use exact onchain values (e.g., `USDC.e` on SKALE Base, `eUSDC` on BITE Sandbox). In your UI you can display a normalized label like `USDC`.
...
Use native confidential tokens with fully shielded balances and transaction amounts that remain private even onchain. Currently available on BITE Sandbox testnet with tokens like `eUSDC`.
