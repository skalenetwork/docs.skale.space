...
2. **Setup hardhat.config file with all SKALE testnets**
```javascript
...
module.exports = {
  ...
  etherscan: {
    ...
    customChains: [
      {
        network: "nebula",
        chainId: 37084624, // Replace with the correct chainId for the "opbnb" network
        urls: {
          apiURL:
      ...
      {
        network: "calypso",
        chainId: 974399131, // Replace with the correct chainId for the "opbnb" network
        urls: {
          apiURL:
      ...
      {
        network: "europa",
        chainId: 1444673419, // Replace with the correct chainId for the "opbnb" network
        urls: {
          apiURL:
      ...
      {
        network: "titan",
        chainId: 1020352220, // Replace with the correct chainId for the "opbnb" network
        urls: {
          apiURL:
...
3. **Create a smart contract under contracts folder**
```solidity
...
```
4. **Create a deploy script under scripts folder**
...
async function main() {
  ...
  const token = await hre.ethers.deployContract("MockContract", [myAddress], {gasLimit});
...
5. **Run the command prompt to call the deploying script**
```sh
npx hardhat run scripts/deploy.js --network <network name you defined on hardhat.config> 
```

6. **Command prompt to verify contract**
```sh
npx hardhat verify --network <network name> <deployed contract address> <contract deployed contructor arguments>
```
</ToolLayout>
