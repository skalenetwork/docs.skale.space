...
The SKALE Network consists of a large pool of validator supernodes, all running concurrently and independently, validating transactions within the SKALE Chains they oversee. These supernodes coordinate with a set of smart contracts that run on the Ethereum mainnet called SKALE Manager. SKALE Manager includes the SKALE Token (SKL) that governs its issuance and how rewards get distributed to validators and delegators.

Developers can rent SKALE Chains for an infinite period of time by paying monthly rent into the network in the form of SKL tokens. These tokens are paid into the Paymaster on the SKALE Europa Hub Mainnet. 

An inflation event also occurs each month whereby new SKALE tokens are created, which gets pushed into the bounty pool for reward to validators and delegators.

For example, suppose there are a thousand validator supernodes in the network and they all perform in accordance with the service parameters. In that case, they will each participate in the monthly proceeds from the bounty pool.

### How does the network work from an operational and governance perspective?

The SKALE Network serves as scaling solution decentralized EVM block space. SKALE makes use of BLS threshold signatures within the network (which allows it to inherit security guarantees from the Ethereum mainnet but provide the performance inherent in a Layer 2 solution).
...
Governance proposals may be issued at any time as per the initial governance proposal to setup onchain governance. You can view that on [Snapshot here](https://snapshot.org/#/s:skale.eth/proposal/0xebbc76cf6bd1afd7e1271f4339c7c04703dbe8dda78b1a731ffaf126772c0051).

### Why does SKALE use the ERC-777 for the SKL token?
...
<Aside>Month 0 is Oct. 2020.</Aside>
...
**[Join the SKALE Developer Discord!](https://discord.gg/skale)**
