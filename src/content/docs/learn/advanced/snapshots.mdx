...
Assumptions:
1. First block on SKALE chain occurred at <math display="inline"><msub><mi>T</mi><mi>firstBlock</mi></msub></math>
2. node does snapshot every <math display="inline"><msub><mi>T</mi><mi>snapshotInterval</mi></msub></math>
seconds (configurable number, stored in SKALE chain config, so it is similar for all nodes in SKALE chain )

Assume `k` snapshots were already done. Lets see when `k+1`-th will be done and ready to be used:
1. `k+1`-th snapshot will be done once another block’s B timestamp crosses boundary <math display="inline"><mo>[</mo><mfrac><msub><mi>T</mi><mi>firstBlock</mi></msub><msub><mi>T</mi><mi>snapshotInterval</mi></msub></mfrac><mo>]</mo><mo>+</mo><mn>1</mn><mo>\*</mo><msub><mi>T</mi><mi>snapshotInterval</mi></msub></math>
