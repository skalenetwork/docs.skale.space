...
SKALE Watchdog is microservice running on every SKALE node for providing SLA agents with node
...
data on all SKALE node docker containers, health checks of sChains and SGX server, Ethereum node endpoint
status, and hardware information.

<Aside type="note">
Node CLI automatically opens port 3009 on the machine using iptables.
Be sure that port 3009 is open for the node’s external network for Watchdog to work.
</Aside>
