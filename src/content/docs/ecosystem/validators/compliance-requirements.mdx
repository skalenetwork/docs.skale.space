...
The following document outlines compliance requirements for SKALE Network nodes.
Failure to follow these requirements will result in a node forfeiting bounty for each month
...
to ensure your node is operating as intended.
...
<Aside type="caution">
Failure to meet ANY ONE of the following Denali compliance requirements will result in a node being
marked “Out of Compliance” by the NODE Foundation and the node may be forced to forfeit monthly bounties
...
- Don't touch machine firewall (iptables installed by skale node software automatically sets machine firewall rules), external
