...
**Required Arguments**
- `--endpoint/-e` is the RPC endpoint of th enode in the network where SKALE Manager is deployed [http or https]
- `--contracts-url/-c` is the URL to SKALE Mnaager contracts ABI and addresses
...
Registers as a new SKALE valdiator.

```shell
sk-val valdiator register
```

**Required Arguments**
- `--name/-n` is the validator name
- `--description/-d` is the validator description
- `--commission-rate/-c` is the commision rate in percentage form
- `--min-delegation` is the minimum delegation amount that the valdiator will accept

**Optional Arguments**
- `--pk-file` is a path to a file with private key (only supported for *software* wallet type)
- `--gas-price` allows the executor to specify th egas price value in gwei for a transaction. If not specified, the average gas price of the network will be double and set.
...
Lists all avaialble validators.
...
**Optional Arguments**
- `--wei/-w` can be used to show th amount of tokens in wei
...
**Required Parameters**
1. `ADDRESS` is the Ethereum address of the valdiator
...
2. `NODE_SIGNATURE` is the signature of the node that you can get using the `skale node signaure` command from the SKALE Node CLI
...
Requests a change of Ethereum Address for the valdiator.
...
Use to cancel a pending delegation reqeust.
...
Request undelegation which goes into affect at the end of the undelegatoin period.
...
Reads the amound of earned bounties by a token holder for a single validator.
...
**Required Arguments**
- `--address-index` is the index of the walle tto use (starts at 0)
- `--keys-type` is the type of Ledger keys whether live or legacy

### Send ETH Tokens

Executes a transfer of ETH tokens to a specfic address.
...
1. `ADDRESS` is the Ethereum reciever address (e.g to)
...
Executes a transfer of SKL tokens to a specific addres.
...
Shows the balance of the valdiator self-recharging wallet.
