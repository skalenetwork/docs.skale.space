...
group of virtualized nodes selected from a subset of the supernodes run by validators in the SKALE network and
are run on all or a subset [multi-tenancy] of each supernode’s computation and storage resources.
...
To encourage proper behavior among network participants, SKALE follows a Proof of Stake (PoS) system whereby each supernode must
