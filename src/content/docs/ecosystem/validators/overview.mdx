...
To encourage proper behavior amongst network participants, SKALE follows a Proof of Stake (PoS) system whereby each node must
...
installed with the [skale-node-cli](/validators/validator-cli). SKALE Admin evaluates the prospective node to ensure that it's upholding
